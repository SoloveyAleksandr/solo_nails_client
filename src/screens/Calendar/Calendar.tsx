    ...
    <div className={styles.Calendar}>
      <Menu isActive={menuIsActive}>
        <ul className={styles.menuList}>
          <li className={styles.menuItem}>
            <NavLink to={"/free-time"}>свободные записи</NavLink>
          </li>
          <li className={styles.menuItem}>
            <NavLink to={"/services"}>услуги и цены</NavLink>
          </li>
          <li className={styles.menuItem}>
            <NavLink to={"/my-reserves"}>мои записи</NavLink>
          </li>
          <li className={styles.menuItem}>
            <NavLink
              onClick={() =>
                reduxDispatch(setSelectedUserUID(appState.currentUserInfo.uid))
              }
              to={"/my-account"}
            >
              мой аккаунт
          ...
          <li className={styles.menuItem}>
            <a className={styles.menuLink} href={"tel: +375257128767"}>
              <span>телефон</span>
              <PhoneIcon w={"18px"} h={"18px"} className={styles.menuIcon} />
            </a>
          </li>
          <li className={styles.menuItem}>
            <a
            ...
            >
              <span>instagram</span>
              <Icon w={"25px"} h={"25px"} className={styles.menuIcon}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
          ...
          <li className={styles.menuItem}>
            <a
              className={styles.menuAdress}
              target={"_blank"}
              href={"https://yandex.by/maps/-/CDF5rGoN"}
            >
      ...
      {appState.selectedMonth.length > 0 ? (
        <>
          <MonthSwitch
            prevMonth={() => {
              reduxDispatch(setPrevMonth());
            }}
            nextMonth={() => {
              reduxDispatch(setNextMonth());
            }}
